  ...
  <div class="bigbox">
    <div class="phoneContent">
      ...
      <template v-if="newActivityForm.discrf==1">
        <div class="crfConten">
          <fm-generate-form :edit="isValid" :data="jsonData" ref="generateForm" :key="jsonData.key"></fm-generate-form>
        </div>
      </template>
      <div class="registerBox">
        <div class="regButton" @click="submit()" v-if="isValid">提交</div>
        <div class="netButton" v-if="!isValid">此活动已失效！</div>
      </div>
      <!-- 注册 -->
      <div
        class="registerBox"
...
export default {
  ...
  mounted() {
    ...
    ///Publics/Common/Dic/IsValidByeventid
    this.hospitalid = this.$route.params.hospitalid;
    this.IsValidByeventid(this.$route.params);
  },
  methods: {
    async submit() {
      let isvaildEvent = false;
      await this.MK.Request(
      ...
      console.log(isvaildEvent);
      if (!isvaildEvent) {
        return;
      }
      //判断crf是否填写
      if (this.activeData.discrf == 1) {
        var [err, resdata] = await to(this.$refs.generateForm.getData());
        if (
          !resdata ||
          !resdata.sendData["04CCAF0B6A4530B1"] ||
          !resdata.sendData["52A1BE983A8788E1"] ||
          !resdata.sendData["7A0FF81C256F88F9"] ||
          !resdata.sendData["F532C11C678DFA49"] |
            !resdata.sendData["1789ABC7C16FACA1"]
        ) {
          this.$message.error("请完善表单");
          return;
        }
      }
      let from = resdata.sendData;
      let newYear = new Date().getFullYear();
      this.regForm.sex = "0"; //性别未知
      this.regForm.membername = from["04CCAF0B6A4530B1"]; //姓名
      this.regForm.age = from["F532C11C678DFA49"]; //年龄
      this.regForm.mobiletel = from["1789ABC7C16FACA1"]; //手机号
      this.regForm.birthday = newYear - this.regForm.age + "0101"; //生日
      this.regForm.idnumber = from["1789ABC7C16FACA1"]; //证件号码 默认为手机号
      this.regForm.cardno = this.NewGuid(); //卡号 默认手机号
