...
/**
 * 获取汉字拼音：传入字符串获取对应汉字首字母
 * @params {Object} 传入字符串类型数据（默认）
 * @return {Object} 返回解析后的汉字首字母
 */
...
Vue.directive('py', {
	bind(el, binding) {
		...
		//去除常见特殊字符串以及数字
		var str = input.value.replace(/[&\|\\\*^%_><~》？、￥…（）｝“”：—《，。,\[\]‘’"】【\/$#!！；｛{@\-^\d+(\.\d+)?$]/g, "");
		//去掉空格
		var str = str.replace(/(^\s+)|(\s+$)/g, "");
		var str = str.replace(/\s/g, "");
		//生成拼音
		var arrRslt = util.py.firstChar(str);
		if (arrRslt.length > 0) {
			input.value = arrRslt;
		}
	}
