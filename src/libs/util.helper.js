 ...
 */
helper.wordUpper = function (word) {
 ...
 */
helper.oneOf  = function(source, need) {
 ...
 * @param {Array} need 需要存在的项目
 */
helper.allIn  = function(source, need) {
 ...
 */
helper.hasChildren  = function(item = {}, keyname = 'children') {
 ...
 */
helper.isIdenticalArray = function(array1, array2) {
  let result = true
 ...
 */
helper.isIdenticalObject  = function(object1, object2) {
  let result = true
  const keys1 = keys(object1)
  const keys2 = keys(object2)
  if (!helper.isIdenticalArray(keys1, keys2)) {
    result = false
  } else {
 ...
 */
helper.isLegalUsername  = function(value) {
 ...
 * @description 适用于表单校验
 */
helper.isLegalUsernameValidator  = function(rule, value, callback) {
 ...
 * @param {String} value 需要校验的数据
 */
helper.isLegalPassword  = function(value) {
 ...
 * @description 适用于表单校验
 */
helper.isLegalPasswordValidator  = function(rule, value, callback) {
 ...
 * @param {String} value 需要校验的数据
 */
helper.isLegalMobilePhone = function(value) {
 ...
 * @description 适用于表单校验
 */
helper.isLegalMobilePhoneValidator  = function(rule, value, callback) {
 ...
 * @param {String} value 需要校验的数据
 */
helper.isLegalEmail = function(value) {
 ...
 * @description 适用于表单校验
 */
helper.isLegalEmailValidator  = function(rule, value, callback) {
  callback(value === '' || helper.isLegalEmail(value) ? undefined : new Error('邮箱格式不正确'))
}
 ...
 */
helper.flatTreeToArray = function({
  data = [],
  ...
  function maker (result, item) {
    ...
    if (helper.hasChildren(item, keyChildren)) result = result.concat(item[keyChildren].reduce(maker, []))
    return result
  }
 ...
 * @param {Object} config {Boolean} includeChildren 输出的数据中是否包含子节点数据
 */
helper.flatTreeToObject = function({
  data = [],
  ...
  function maker (result, item) {
    ...
    if (helper.hasChildren(item, keyChildren)) Object.assign(result, item[keyChildren].reduce(maker, {}))
    return result
  }
  return data.reduce(maker, {})
}
 ...
 */
helper.getNumberOrZero  = function(value) {
