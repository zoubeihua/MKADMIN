	...
	export default {
		...
		methods: {
			...
			async model_form_cofig() {
				...
				console.log('res++++++++++')
				console.log(res)
				//code -1:没有对应的控件类型 1:本地组装表单数据 2:本地与网络合并表单数据 3:只返回网络表单数据
				if(res.code == -1 ){
					this.$XModal.message({
						status: 'error',
						message: '没有对应的控件类型',
			...
			async mergeMakingFormData(){
				...
				return new Promise(async (resolve) => {
					let netWidgetForm,WidgetFormOption;
					let res = await this.getNetMakingFormData();
					if(res.code == 0){
						let data = res.data;
						if(data.length == 0){
							WidgetFormOption = await this.WidgetFormOption();
							if(!WidgetFormOption){
								resolve({
									code:-1,
									WidgetForm:this.modelFromConfig
								})
							}else{
								resolve({
									code:1,
									WidgetForm:WidgetFormOption
								})
							}
						}else{
							WidgetFormOption = await this.WidgetFormOption(); //本地组装
							console.log('本地组装')
							console.log(WidgetFormOption)
							netWidgetForm = JSON.parse(data[0].introinfo);//网络请求数据 
							console.log('网络请求数据')
							console.log(netWidgetForm)
							if(WidgetFormOption){
								let { queryWidgetForm, addWidgetForm, modifyWidgetForm} = WidgetFormOption;
								let { queryWidgetForm:netqueryWidgetForm, addWidgetForm:netaddWidgetForm, modifyWidgetForm:netmodifyWidgetForm} = netWidgetForm;
								let WidgetForm = {
									queryWidgetForm:this.merge(queryWidgetForm,netqueryWidgetForm),
									addWidgetForm:this.merge(addWidgetForm,netaddWidgetForm),
									modifyWidgetForm:this.merge(modifyWidgetForm,netmodifyWidgetForm)
								}
								resolve({
									code:2,
			...
			//服务器数据替换本地数据
			merge(locaData,netData){
				netData.list.forEach((item,i) => {
					locaData.list.forEach((data,j) => {
						if(item.model == data.model)
						{
							locaData.list[j] = JSON.parse(JSON.stringify(item))
						}
					})
				})
				locaData.config = JSON.parse(JSON.stringify(netData.config))
				return locaData;
			},
			//获取弹窗表单和查询表单配置
			...
			async WidgetFormOption(){
				...
				let addList = [];
				let editList = [];
				let componentsDataClone = JSON.parse(JSON.stringify( this.componentsData));
				if(tableData.length == 0) return false;
				for (let i = 0; i < tableData.length; i++) {
					let row = tableData[i];
					if(row.fieldtype){
						if (row.is_query) {
							let queryRow = await this.getRow(componentsDataClone,row,'query');
						...
						if (row.is_add) {
							let addRow =  await this.getRow(componentsDataClone,row,'add');
							if(!addRow) return false;
							addList.push(addRow);
						}
						if (row.is_modify) {
							let editRow = await this.getRow(componentsDataClone,row,'edit');
							if(!editRow) return false;
							editList.push(editRow);
						}
					}
					
				}
				let querylist = _.compact(queryList);
				let addlist = _.compact(addList);
				let modifylist = _.compact(editList);
				let queryWidgetForm = this.getWidgetForm(querylist);
				let addWidgetForm = this.getWidgetForm(addlist);
				let modifyWidgetForm = this.getWidgetForm(modifylist);
				return {
					queryWidgetForm,
					addWidgetForm,
					modifyWidgetForm
			...
			/* 
			 componentsData 模板数据
			 row 当前grid行数据
			 type 表单类型 query add edit 三种类型 来区分key
			 */
			async getRow(componentsData,row,type){
				let curRow = componentsData.filter(item => item.type == row.fieldtype)[0];
				if(!curRow) return false;
				if(row.dic && row.dic != ''){
					if(curRow.type == 'radio' || curRow.type == 'checkbox' || curRow.type == 'select'){
						let res = await this.$getDicCode(row.dic);
						curRow.options.options = res;
					};
				};
				curRow.model = row.key || 'name';
				curRow.key =  new Date().getTime() + '_' + row.key + '_' + type;
				curRow.name = row.name || 'title';
				return JSON.parse(JSON.stringify(curRow));
			},
			getWidgetForm(list = []) {
				return {
					list: list,
