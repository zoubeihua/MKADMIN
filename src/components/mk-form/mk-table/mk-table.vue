              ...
              <div v-if="column.edit.keep|| edit.rowIndex==scope.$index" class="e-item">
                <div>
                  <el-date-picker
					...
					:value-format="column.edit.type=='date'? 'yyyy-MM-dd':'yyyy-MM-dd HH:mm:ss'"
...
<script>
var $vue;
import MkTableRender from "./MkTableRender.js";
import util from "@/libs/util.js";
export default {
  components: { "table-render": MkTableRender },
  props: {
    tableData: {
  ...
  data() {
    return {
      ...
      defaultImg: 'this.src="' + require("@/assets/imgs/touxiangnan.png") + '"',
      loading: false,
      footer: {},
      total: 0,
      formatConfig: {},
  ...
  methods: {
        ...
        if (file.indexOf(".") != -1) {
          ...
          if (splitFile.length > 0) {
            fileInfo.push({
              ...
              path: util.base.isUrl(file) ? file : this.http.ipAddress + file
            });
          }
        }
      }
    ...
    viewImg(row, column, url) {
      util.base.previewImg(url, this.http.ipAddress);
